  ...
  head: () => ({
    meta: [
      { title: "Governance — JanSeva Protocol" },
      {
        name: "description",
        content:
          "JanSeva Protocol is governed by a distributed stewardship council. No single entity controls it.",
      },
    ...
    (async () => {
      const [tot, ans, chain, cats, c, v] = await Promise.all([
        ...
        supabase
          ...
          .eq("ai_status", "answered"),
        supabase
          .from("submissions")
          .select("*", { count: "exact", head: true })
          .neq("blockchain_tx_hash", ""),
      ...
      <main className="flex-1 px-4 sm:px-6 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Principle
              title="No single point of capture"
              body="The protocol is designed so no government, corporation, or individual can shut it down or alter records."
            />
            <Principle
              title="Radical transparency"
              body="Every submission, every AI response, every governance vote is permanently public."
            />
            <Principle
              title="Distributed stewardship"
              body="Seven independent stewards from different countries — none with unilateral control."
            />
          </div>
          ...
          {byCat.length > 0 && (
            <>
              <h2 className="font-display text-2xl text-foreground mt-12 mb-4">
          ...
          <div className="mt-12 card-editorial p-6 sm:p-8 text-center bg-primary text-primary-foreground border-primary">
            ...
            <p className="text-primary-foreground/80 text-sm mb-4 max-w-md mx-auto">
              JanSeva is MIT licensed. Fork it, audit it, run your own instance. We can't stop you —
              that's the point.
            </p>
            <a
              href="#"
